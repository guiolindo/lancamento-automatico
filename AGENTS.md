# Guia pra agentes de IA (e humano novo no projeto)

<!--
Este arquivo é o `AGENTS.md` — convenção agente-agnóstica adotada por
Codex, Cursor, Aider, Manus, Grok Studio e outros. Também é linkado
pelo `CLAUDE.md` (stub) pra Claude Code encontrar automaticamente.
Se você estiver usando outra ferramenta que carrega instruções por
convenção específica (`.github/copilot-instructions.md`,
`.cursor/rules/`, `.windsurfrules`, `.gemini/`, etc.), adicione um
arquivo lá apontando pra este.
-->
