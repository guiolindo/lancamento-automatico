...
| 92 | Check de rede corporativa no launcher: se `[REDACTED-DOMAIN]` (DNS interno da Economart) não resolve em 3s, o app sai com aviso "Conecte à rede corporativa ou VPN". Roda ANTES de mutex/updater/Qt — falha rápido, sem trabalho perdido. Bypass pra dev: `AUTOCONFERI_DEV=1`. Zero AV concern (`socket.gethostbyname` é chamada DNS padrão que Chrome/Windows/Outlook fazem centenas por dia). | `launcher.py` |
