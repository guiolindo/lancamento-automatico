...
| 82 | Download de update no boot vira SILENCIOSO — sem modal "Sim, atualizar agora". Ao detectar versão nova, chama direto `_updater_bar.iniciar(info)` e loga a linha na ATIVIDADE. Só a barra laranja no topo mostra o progresso. `_modal_obrigatorio` deletado (dead code). Botão manual "Atualizar" também dispara direto — clicar já é a confirmação. | `main_window.py` |
...
| 89 | Novo imposto **INSS** + arquitetura extensível: (a) `mapeamento.json` ganha entry INSS (espécie MINSS, pessoa 48891, template "INSS SOBRE FOLHA - REFERENTE A: {mes_ref}/{ano_ref}"); (b) novo campo `Imposto.mes_ref_regra` — quando `="mes_anterior_emissao"` o `montar_lancamentos` calcula `mes_ref/ano_ref = data_emissao - 1 mês` em vez de usar o do PDF (INSS: emissão 20/07/26 → observação "REFERENTE A: 06/2026"); (c) combo IMPOSTO da UI vira dinâmico — puxa `mapping.impostos_disponiveis()` em vez do hardcoded "IRRF". Adicionar imposto novo = editar JSON, sem tocar em Python. | `mapeamento.json`, `models.py`, `mapping.py`, `gemini_client.py`, `main_window.py` |
| 90 | Prompt Gemini reestruturado em **base compartilhado + específico por imposto**. `PROMPT_BASE` tem catálogo + regras universais (normalização de nome, ambiguidades, formato de valores, JSON de resposta). Cada imposto tem sua seção em `PROMPTS_POR_IMPOSTO[chave]` — o do IRRF explica "múltiplas colunas por tipo de folha", o do INSS explica "coluna única VALOR + não tente adivinhar mês/ano". `_construir_prompt(imposto, mapping)` junta base + específico. Fix na **ambiguidade Feira de Santana** (v1, incompleto — ver 91). | `gemini_client.py`, `mapeamento.json` |
| 91 | Ambiguidade LOJA/CD generalizada — o build-90 tinha regra "sem qualificador = CD" hardcoded, mas existem 2 convenções válidas dependendo do relatório: **Convenção A** marca a LOJA com "(LOJA)" e bare = CD; **Convenção B** marca o CD com "CD" prefix e bare = LOJA. Aliases da filial 201 restritos a "CD FEIRA...", removido "FEIRA DE SANTANA" solto (que travava a convenção B). Prompt reescrito: ensina as duas convenções e pede pra Gemini varrer o relatório INTEIRO antes de decidir qual convenção esse doc usa. Se não achar nenhum marcador, devolve texto bruto e cai no fuzzy match. Vale pra qualquer cidade com loja+CD no catálogo (Ribeirao das Neves também). | `gemini_client.py`, `mapeamento.json` |
...
| 93 | MessageBox e BUILD_MARKER (release notes públicas) não mencionam mais o nome específico do DNS interno. Mensagem ao usuário fica genérica ("rede corporativa" em vez de "intranet X.intranet"). O nome do DNS ainda vive no código-fonte (`REDE_DNS_INTERNO`); pra removê-lo de vez, precisaria mover pra hash+salt ou pra variável de ambiente do sistema (`USERDNSDOMAIN` — proposta pra build futuro se necessário). | `launcher.py`, `main.py` |
